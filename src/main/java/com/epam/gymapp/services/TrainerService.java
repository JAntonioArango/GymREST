...
public class TrainerService {
    ...
    public TrainerRegistrationDto register(CreateTrainerDto dto) {
        ...
        if (traineeRepo.existsByUserFirstNameAndUserLastName(
        ...
        User u = userRepo.findByUsername(saved.username()).orElseThrow(() ->
                ApiException.notFound("User", saved.username()));

        return new TrainerRegistrationDto(
                u.getUsername(),
                u.getPassword());
    }
    ...
    public TrainerDto findByUsername(String username) {
        return trainerRepo.findByUserUsername(username)
                .map(this::toDto)
                .orElseThrow(() ->
                        ApiException.notFound("Trainer", username));
    }
    ...
    private void checker(CreateTrainerDto dto, Trainer trainer) {
        if (dto.specialization() != null &&
            ...
            TrainingType spec = typeRepo.findByName(dto.specialization())
                    .orElseThrow(() ->
                            ApiException.notFound("Training type", dto.specialization()));
    ...
    private TrainerProfileDto toProfileDto(Trainer t) {
        ...
        List<TraineeShortDto> traineeDtos = t.getTrainees().stream()
                .map(tr -> new TraineeShortDto(
    ...
    public List<TrainerShortDto> unassignedActiveTrainers(String traineeUsername) {
        ...
        Set<Long> assignedIds =  trainerRepo
