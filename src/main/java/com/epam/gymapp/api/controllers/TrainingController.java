...
public class TrainingController {
    ...
    public ApiListWrapper<TraineeTrainingDto> traineeTrainings(
        ...
        var filter = new TrainingService.TrainingFilter(
                from, to, trainerName, null, trainingType);

        Page<TraineeTrainingDto> p = service.traineeTrainings(
                username, filter, PageRequest.of(page, size));
    ...
    public ApiListWrapper<TrainerTrainingDto> trainerTrainings(
        ...
        var filter = new TrainingService.TrainingFilter(
                from, to, null, traineeName, null);

        Page<TrainerTrainingDto> p =
                service.trainerTrainings(username, filter,
    ...
    public ApiListWrapper<TrainingTypeDto> listTypes(
            @AuthenticatedUser User caller) {
    ...
    private static TrainingService.TrainingFilter getTrainingFilter(LocalDate from, LocalDate to, String trainerName, String trainingType) {
        TrainingService.TrainingFilter filter = (from == null && to == null &&
                blank(trainerName) && blank(trainingType))
                ? null
                : new TrainingService.TrainingFilter(
                from, to, trainerName, null, trainingType);
        return filter;
    }
    ...
    private static boolean filterMatch(TrainingDto t, TrainingService.TrainingFilter f) {
        ...
        boolean traineeOk = f.traineeNameOrNull() == null ||
                t.traineeUsername().toLowerCase().contains(
                        f.traineeNameOrNull().toLowerCase());

        return dateOk && traineeOk;
    }

    private static boolean isDateOk(TrainingDto t, TrainingService.TrainingFilter f) {
        boolean dateOk = (f.fromDate() == null || !t.trainingDate().isBefore(f.fromDate())) &&
